package com.marley.todolist.tasks;
...
//
import com.marley.todolist.utils.Utils;
//
...
public class TaskController {
    ...
    public ResponseEntity Create(@RequestBody TaskModel taskModel, HttpServletRequest request) {
        ...
        }//case current date greater or less than  start date/end date
    ...
    public ResponseEntity update(@RequestBody TaskModel taskModel, HttpServletRequest request, @PathVariable UUID taskId){
        ...
        if(task == null){
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("this task does not exist");
        }

        var idUser =  request.getAttribute("idUser");
        if(!task.getIdUser().equals(idUser)){
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("this task does not belong to this user");
        }
